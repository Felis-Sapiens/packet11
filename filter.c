        ...
        )
{
    ...
    ASSERT(NdisFilterDriverHandle == FilterDriverHandle);                       // line 186
    ASSERT(FilterDriverContext == (NDIS_HANDLE)FilterDriverObject);             // line 187
...
--*/
{
    ...
    NET_BUFFER_POOL_PARAMETERS      Parameters1;
    int                             n = 0;
    int                             i;
    int                             v_llg6y;
    int                             v_0Dfjc;
    ...
    do
    {
        ASSERT(FilterDriverContext == (NDIS_HANDLE)FilterDriverObject);         // line 259
        ...
        pFilter = (PMS_FILTER)FILTER_ALLOC_MEM(NdisFilterHandle, Size);         // line 272
        if (pFilter == NULL)
        {
        ...
        if (AttachParameters->MiniportMediaType == NdisMediumNative802_11)
            DEBUGP(DL_WARN, ("MSFilter: Miniport below supports NdisMedium802_11.\n"));

        if (AttachParameters->MiniportMediaType == NdisMedium802_3)
            DEBUGP(DL_WARN, ("MSFilter: Miniport below supports NdisMedium802_3.\n"));
        
        pFilter->MiniportMediaType = AttachParameters->MiniportMediaType;
        pFilter->MediaConnectState = AttachParameters->MediaConnectState;
        NdisMoveMemory(&pFilter->CurrentMacAddress, AttachParameters->CurrentMacAddress, 6);
        pFilter->field_294 = 0;
        pFilter->field_172 = 0;
        pFilter->field_173 = 0;
        pFilter->field_174 = 1;
        ...
        pFilter->field_114[0] = 1;
        pFilter->field_134[0] = 1;
        pFilter->field_154[0] = 1;
        pFilter->field_16A[0] = 1;
        pFilter->field_16E[0] = 1;
        pFilter->pool_derived_from_nbl = 0;
        ...
        p.DataSize = 0;
        p.PoolTag = 'tliF';
        pFilter->SendNblPool = NdisAllocateNetBufferListPool(NdisFilterHandle, &p);
        if ( pFilter->SendNblPool == NULL)
        {
            DEBUGP(DL_WARN, ("AttachFilter: failed to alloc send net buffer list pool\n"));
            Status = NDIS_STATUS_RESOURCES;
            break;
        }
        p.ContextSize = 0;
        pFilter->RecvNblPool = NdisAllocateNetBufferListPool(NdisFilterHandle, &p);
        if ( pFilter->RecvNblPool == NULL)
        {
            DEBUGP(DL_WARN, ("AttachFilter: failed to alloc recv net buffer list pool.\n"));
            Status = NDIS_STATUS_RESOURCES;
            break;
        }
        NdisZeroMemory(&Parameters1, sizeof(NET_BUFFER_POOL_PARAMETERS));
        Parameters1.Header.Type = 0x80;
        Parameters1.Header.Revision = 1;
        Parameters1.Header.Size = 12;
        Parameters1.DataSize = 0;
        Parameters1.PoolTag = 'tliF';
        pFilter->SendNbPool = NdisAllocateNetBufferPool(NdisFilterHandle, &Parameters1);
        if ( pFilter->SendNbPool == NULL)
        {
            DEBUGP(DL_WARN, ("AttachFilter: failed to alloc send net buffer pool\n"));
            Status = NDIS_STATUS_RESOURCES;
            break;
        }
        pFilter->CancelId = NdisGeneratePartialCancelId();
        pFilter->CancelId <<= 24;
        DEBUGP(DL_LOUD, ("AttachFilter: CancelId %lx\n", pFilter->CancelId));
...
--*/
{
    ...
    // 
    FILTER_ASSERT(pFilter->State == FilterRunning);                             // line 536
...
--*/
{
    ...
    FILTER_ASSERT(pFilter->State == FilterPaused);                              // line 592
    ...
    else if ( pFilter->MiniportMediaType == NdisMediumNative802_11 )
    {
      f = NDIS_PACKET_TYPE_PROMISCUOUS 
...
void FilterFreePools(PMS_FILTER pFilter)
{
  if ( pFilter->SendNblPool )
  {
    NdisFreeNetBufferListPool(pFilter->SendNblPool);
    pFilter->SendNblPool = NULL;
  }
  if ( pFilter->RecvNblPool )
  {
    NdisFreeNetBufferListPool(pFilter->RecvNblPool);
    pFilter->RecvNblPool = NULL;
  }
}
...
--*/
{
    ...
    // 
    FILTER_ASSERT(pFilter->State == FilterPaused);                              // line 789
...
#if DBG    
    FILTER_ACQUIRE_LOCK(&FilterListLock, FALSE);
    ASSERT(IsListEmpty(&FilterModuleList));                                     // line 856
...
--*/
{
    ...
    do
    {
        if ( pFilter->MiniportMediaType == NdisMedium802_3 )
            v_TVMrP = Request->RequestType;
        if ( pFilter->MiniportMediaType == NdisMediumNative802_11 )
        {
            v_qlLvh = Request->RequestType;
            if ( v_qlLvh != NdisRequestSetInformation )
            {}
            else if (Request->DATA.QUERY_INFORMATION.Oid == OID_DOT11_EXCLUDE_UNENCRYPTED )
            {
...
--*/
{
    ...
    ASSERT(pFilter->PendingOidRequest == Request);                              // line 1187
...
#if DBG
    FILTER_ACQUIRE_LOCK(&pFilter->Lock, FALSE);
    ASSERT(pFilter->bIndicating == FALSE);                                      // line 1265
...
#if DBG    
    FILTER_ACQUIRE_LOCK(&pFilter->Lock, FALSE);
    ASSERT(pFilter->bIndicating == TRUE);                                       // line 1309
...
--*/
{
    ...
    if (pFilter->TrackSends)
    {
        ...
        while (CurrNbl)
        {
            pSendContext = NET_BUFFER_LIST_INFO( CurrNbl, MediaSpecificInformation );
            if ( pSendContext && pSendContext->uSendFlags == 1 )
            {
                q = TRUE;
                break;
            }
...
--*/
{
    ...
    do
    {
       ...
       DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(SendFlags);
        ...
        {
            DEBUGP(DL_WARN, ("===>SendNetBufferList: filter not running \n"));
            ...
            while (CurrNbl)
            {
                ...
                if ( pFilter->pool_derived_from_nbl == NULL)
                    FilterNetBufferList(CurrNbl, pFilter->MiniportMediaType, pFilter, 1);
                CurrNbl = NET_BUFFER_LIST_NEXT_NBL(CurrNbl);
            }
        ...
        if (!q || v_q9elU == NULL)
            ...
            NdisFSendNetBufferLists(pFilter->FilterHandle, v_q9elU, PortNumber, SendFlags);
...
--*/
{
    ...
    do
    {
        ...
        if (pFilter->State != FilterRunning)
        {
            DEBUGP(DL_TRACE, ("===>ReceiveNetBufferList: filter is not running.\n"));
            
            CurrNbl = NetBufferLists;
            while (CurrNbl)
            {
                pFilter->OutstandingRcvs += NumberOfNetBufferLists;
                FilterNetBufferList(CurrNbl, pFilter->MiniportMediaType, pFilter, 0);
                CurrNbl = NET_BUFFER_LIST_NEXT_NBL(CurrNbl);
            }
            FILTER_RELEASE_LOCK(&pFilter->Lock, DispatchLevel);
        ...
        ASSERT(NumberOfNetBufferLists >= 1);                                    // line 1786
